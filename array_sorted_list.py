...
Updated by: Benjamin Leong Tjen Ho
"""
...
class PotionSortedList(ArraySortedList):
    def __setitem__(self, index: int, value: T) -> None:
        """ Magic method. Insert the item at a given position,
            if possible (!). Shift the following elements to the right.
        """
        if self.is_empty() or \
                (index == 0 and value[0] <= self[index][0]) or \
                (index == len(self) and self[index - 1][0] >= value[0]) or \
                (index > 0 and self[index - 1][0] >= value[0] >= self[index][0]):

            if self.is_full():
                self._resize()

            self._shuffle_right(index) # O(n)
            self.array[index] = value
        else:
            # the list isn't empty and the item's position is wrong wrt. its neighbourghs
            raise IndexError('Element should be inserted in sorted order')

    def potion_add(self, item: tuple) -> None:
        """ Add new element to the list. """
        if self.is_full():
            self._resize()

        # find where to place it
        position = self._potion_index_to_add(item)

        self[position] = item
        self.length += 1

    def _potion_index_to_add(self, item: tuple) -> int:
        """ Find the position where the new item should be placed.
        Algorithm adjusted to sort highest to lowest.
        """
        low = 0
        high = len(self) - 1
        ...
        while low <= high:
            ...
            if self[mid][0] > key:
                ...
                if self[mid][1] < backup:
                    return mid + 1
                elif self[mid][1] > backup:
                    return mid
        return low

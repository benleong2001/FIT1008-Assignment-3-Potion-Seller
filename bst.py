...
class BinarySearchTree(Generic[K, I]):
    ...
    def get_successor(self, current: TreeNode) -> TreeNode:
        ...
        Idea is :
        1) since it is a successor, go right of the current node once
        2) then from there on take the values on the left of
        so its like
                          --10--
                         |      |
                        5    --20--
                             |     |
                            19     21
        pick 19 as successor      
        
        :param current:     The node in the current subtree.
        :return:            The node which is greater then the current node
        :complexity:        O(log(n)) where n is the number of nodes
        :pre:               Input current must be an TreeNode.
        :raises ValueError: When input current is not an TreeNode.

        ----------------------------------------------------------------------------------------------------------------
        METHODS CALLED                  |   COMPLEXITY  |   REMARKS
        --------------------------------|---------------|---------------------------------------------------------------
        isinstance()                    |   O(IsIns)    |   Unknown complexity for built-in function. Assumed to be O(1)
        BinarySearchTree.get_minimal    |   O(log(n))   |
        --------------------------------|---------------|---------------------------------------------------------------
        ----------------------------------------------------------------------------------------------------------------
        """
        # Checking pre condition(s)
        if not isinstance(current, TreeNode):
            raise TypeError("".join(["Parameter current must be a TreeNode: current = ", str(current)]))

        # Step 2: Go to right child (if any)
        if current.right is not None:
            successor_node = current.right

        # If no right child, it has no successor (so, return None)
        else:
            return None

        # Step 3: Go to the left until a leaf is reached
        successor_node = self.get_minimal(successor_node)
    ...
    def get_minimal(self, current: TreeNode) -> TreeNode:
        ...
        :param current:     The node in the current subtree.
        :return:            The smallest node which is also the leaf
        :complexity:        O(log(n)) where n is the number of nodes
        :pre:               Input current must be an TreeNode.
        :raises ValueError: When input current is not an TreeNode.
        ----------------------------------------------------------------------------------------------------------------
        ...
        # Left children are always smaller than current
        # So, it keeps going left until there is no left child
